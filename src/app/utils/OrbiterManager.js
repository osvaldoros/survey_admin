...
define([
	...
	var classRef = declare("app.utils.OrbiterManager", [], {
		...
		orbiterReady:function(event){
			...
			this.hashChangeHandle = dojo.subscribe("/emanda2/hashchange", this, this.onHashChange);
			...
			this.systemRoom.addEventListener(net.user1.orbiter.RoomEvent.UPDATE_CLIENT_ATTRIBUTE, this.clientAttributeUpdateListener, this);
			this.systemRoom.addMessageListener("PROGRESS", this.messageListener, this);
		...
		notificationListener:function(fromClient, message){
			...
			console.log(message)			
			var data = json.fromJson(message);
			topic.publish("push-" + data.type, data);
		},
