...
define("app/uicomponents/Notes", [
	...
	function(declare, domClass, domConstruct, domStyle, on, lang, template, WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, StatefulModule,
		...
		return declare("app.uicomponents.Notes", [WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, StatefulModule, DCFormManager], {
			...
			refreshActivity:function(entity, autoShow){
				console.log("refreshActivity > ")
				if(typeof(autoShow) == "undefined") autoShow = false;
				...
				var res = __.api.list(
					...
					function(activities){
						...
						if(autoShow === true) owner.autoShow(activities)
					},
					// base_query
					{entity_id: entity}
				);
			...
			autoShow:function(activities){
				console.log("autoShow > ")
				if(lang.isArray(activities) && activities.length > 0 && !this.__isShowing()){
					this.show();
				}else{
					this.hide();
				}
			...
			__isShowing:function(){
				return this.parentPane.__isShowing;
			},

			show:function(){
				console.log("show >")
				if(typeof(this.parentPane) == "object" && this.parentPane != null){
					console.log("parentPane > show > ")
					this.parentPane.show();	
					this.parentPane.bringToTop();
				}
			},
			...
			hide:function(){
				console.log("hide >")
				if(typeof(this.parentPane) == "object" && this.parentPane != null){
					console.log("parentPane > hide > ")
					this.parentPane.hide();	
				}
