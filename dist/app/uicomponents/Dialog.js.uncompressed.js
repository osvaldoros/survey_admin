 ...
 * 
 */
define("app/uicomponents/Dialog", [
	"dojo/_base/declare",
	"dijit/Dialog",
	...
	return declare([Dialog, StatefulModule], {
		...
		setUpdatingEntity:function(entity, entityInjected){
			...
			for (var i=0; i < children.length; i++) {
				var childNode = children[i];
				if(typeof(childNode.setUpdatingEntity) == 'function'){
					childNode.setUpdatingEntity(entity, entityInjected);
				}
			}
		},

		/*
		NOTE that this only returns the FIRST getUpdatingEntity that it finds within its children
		In theory this should be fine, since setUpdatingEntity, sets the same entity in all of them...
		*/
		getUpdatingEntity:function(){
			var children = this.getChildren();
			for (var i=0; i < children.length; i++) {
				var childNode = children[i];
				if(typeof(childNode.getUpdatingEntity) == 'function'){
					return childNode.getUpdatingEntity();
				}
			}
		},			
