...
define("app/mixins/TabManager", [
	...
	"app/mixins/DynamicFormModule"
	
	],
	function(declare, on, lang, baseArray, topic, domClass, HashManager, ModuleLoader, DialogLauncher, DynamicFormModule){
	...
	return declare([], {
		...
		configureTabs:function(configObject){
			...
			if(typeof(configObject._showTabs) != "undefined") this._showTabs = configObject._showTabs;
			
			this.configureHashManagement(configObject)

			/*========================
			* handle dialogs that are being popped from within a tabManager. When the dialog is closed we need to make sure the state is set to whatever the tab "owns" the dialog
			*=========================
			*/
								...
								for (var j = classArray.length - 1; j >= 0; j--) {
									...
									for (var l = configObject.alternateStates.length - 1; l >= 0; l--) {
										...
										if(typeof(alternateState.name) == "string" && alternateState.name.toLowerCase() == clazz.toLowerCase()){
											child.set("closeToState", alternateState.mapsTo + "/");
											childMapped = true;
										}
		...
		/*
		*
		* Updates the policy to update the hash or not based on configuration
		*/
		configureHashManagement:function(configObject){
		...
		onActivate:function(){
			...
			// creates a moduleLoader for each module and adds it as a child of the tabs
			this.createTabs();
		...
		createTabs:function(){
			...
			if(typeof(this.overviewStep) != undefined && this.overviewStep != null && this.isUpdating){
			...
			for(var i=0,j=this._currentSteps.length; i<j; i++){
				...
				moduleObj.onActivateCallBack = function(moduleName){
					return function(){
						owner.childActivated(moduleName);
					}	
				}(moduleName); 
				
				
				moduleObj.parentStack = this.__tabContainer;
				moduleObj.parentWizard = this;
			...
			var visibleChild;
			if(childToSelect != null && this.isUpdating){
				this.__tabContainer.selectChild( childToSelect );
				visibleChild = 	childToSelect;
			}
			else{
				visibleChild = 	firstModule;	
			}
		 ...
		 * 
		 */
		childActivated:function(moduleName){
