...
define("app/mixins/Saver", [
	...
	return declare([CallBacks], {
		...
		configureSaver:function(configObject){
			...
			if(typeof(configObject.storeURL) == "string"){
				this.__storeURL = configObject.storeURL;
			}else{
				this.__storeURL = "NO_STORE_" + this.id; // to allow multiple local changes being tracked independetly append the id
			}
		},

		getStoreURL:function(){
			return this.__storeURL;
		},
		
		
		setUpdatingEntity:function(entity, entityInjected){
			this.updatingEntity = entity;
			this.entityInjected = entityInjected;
		},
		...
		shouldCommitOnSave:function(){
			if(typeof(this.__storeURL) == "string"){
				return (this.__storeURL.indexOf("NO_STORE") == -1);
			}
