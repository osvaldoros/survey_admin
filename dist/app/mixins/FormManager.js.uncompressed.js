...
define("app/mixins/FormManager", [
	...
	return declare([], {
		...
		startup:function(){
			...
			for (var i = children.length - 1; i >= 0; i--) {
				...
				if(typeof(child.isInstanceOf) == "function" && child.isInstanceOf(Manager)){
					this.__form = child;
					this.setStoreFromParentWizard(this.__form);
				}
			};
		...
		setStoreFromParentWizard:function(form){
			if(typeof(this.__form.storeURL) == "undefined"){
				if(typeof(this.parentWizard) == "object" && this.parentWizard != null && typeof(this.parentWizard.getStoreURL) == "function"){
					var newStoreURL = this.parentWizard.getStoreURL();
					if(newStoreURL != null && typeof(newStoreURL) != "undefined"){
						this.__form.storeURL = newStoreURL;
					}else{
						console.warn("Form Manager " + this.id + " without storeURL!");
					}
				}
			}
		},
		...
		gatherFormValues:function(names){
			if(typeof(this.__form) != "undefined" && this.__form != null && typeof(this.__form.gatherFormValues) == "function"){
				return this.__form.gatherFormValues(names);
			}
		},
		...
		onReloadEntity:function(freshEntity){
			if(typeof(this.__form) != "undefined" && this.__form != null){
				var entityToLoad = freshEntity ? freshEntity : this.getUpdatingEntity();
				this.viewInForm(this.getUpdatingEntity(), this.__form);
			}
		},

		getFromParent:function(name){
			if(typeof(this.parentWizard) == "object" && this.parentWizard != null){
				return this.parentWizard[name];
			}
		},

		setOnParent:function(name, value){
			if(typeof(this.parentWizard) == "object" && this.parentWizard != null){
				this.parentWizard[name] = value;
			}
		},			
		...
		/**
		 *
		 * Used to get the currently updating entity from the parent wizard 
		 *
		 */
		getUpdatingEntity:function(){
			if(typeof(this.parentWizard) == "object" && this.parentWizard != null && typeof(this.parentWizard.getUpdatingEntity) == "function"){
				return this.parentWizard.getUpdatingEntity();
			}
			
			var localUpdatingEntity = this.inherited(arguments);

			return localUpdatingEntity;
		},		
		...
		onActivate:function(){
			...
			if(this.__form){
				...
				if(this.parentWizard){
					this.parentWizard.currentForm = this; //Lets WizardManager block that I'm the current form as instantiated object
				}
			}
		},
		 ...
		 * 
		 */
		viewInForm:function(object, form, arrayPropIndexes, reset){
		...
		flattenNames:function(object, flattened, arrayPropIndexes, path){
			...
			for(var p in object){
				...
				if(lang.isArray(prop)){
					...
					if(arrayPropIndexes != null && typeof(arrayPropIndexes) != "undefined" && arrayPropIndexes.hasOwnProperty(p) && typeof(prop[arrayPropIndexes[p]]) != "undefined"){
					...
					if(typeof(prop.$ref) != "undefined" || typeof(prop.ref) != "undefined"){
